...
class ApidetProducController extends Controller
{
    ...
    public function detialsprod(Request $request)
    {
        ...
        $list['card_image'] = URL::to($sec->image);
        ...
        if (count($copies) == 0) {
            ...
            foreach ($copies as $key => $sec) {
                ...
                $list['copies_products'][$key]['image'] = URL::to('uploads/products_images/' . $sec->image);

                $list['copies_products'][$key]['name'] = $name;
                $list['copies_products'][$key]['rate'] = $sec->rate;
                $list['copies_products'][$key]['old_price'] = $sec->price;

                $list['copies_products'][$key]['card_image'] = URL::to('uploads/products_images/' . $sec->card_image);
                $list['copies_products'][$key]['card_image'] = URL::to('uploads/products_images/' . $sec->image);
        ...
        if (count($sec->Images) == 0) {
            ...
            $productImages = Product_Image::where('product_id', $request->input("id"))->get();
            ...
            //                if($image->product_id == $request->input("id") ) {
            //                    $list['Images'][$key]['id'] = $image->id;
            //                    $list['Images'][$key]['Image'] = URL::to('uploads/products_images/' . $image->image);
            //                } else {
            //                    continue;
        ...
        if ($request->header('kind') == 'd') {
            ...
            if ($order) {
                ...
                if ($product->stock < $request->count) {
                    return response()->json(['message' => 'Quantity Is more than product stock'], 200);
                }
                ...
                if ($checkIfProductExistsInCart) {
                    ...
                    if (!empty($checkIfProductExistsInCart->count)) {
                        $userWantedQuantity = $checkIfProductExistsInCart->count + $request->count;
                        $ProductCurrentQuantity = $product->stock;
                        if ($ProductCurrentQuantity < $userWantedQuantity) {
                            // Return To The User With A Response Because He Wants Quantity More Than In The Stock
    ...
    public function countcart(Request $request)
    {
        ...
        if (!is_null($product->stock)) {
            if ($product->stock < $request->count) {
                ...
                return response()->json(['status' => 200, 'message' => 'Quantity Is more than product stock', 'data' => $list], 200);

            }
        } else {
            // return response()->json(['message' => 'Product Quantity Is Not Available'],400);
            return response()->json(['status' => 200, 'message' => 'Product Quantity Is Not Available',], 200);
        }
        ...
        if (count($order->Carts) == 0) {
            ...
            foreach ($order->Carts as $key => $val) {
                ...
                $list['Carts'][$key]['price_old'] = $val->Product->price;
                if ($list['Carts'][$key]['id'] == $request->id) {
                    $list['Carts'][$key]['count'] = (int) $request->count;
                } else {
                    $list['Carts'][$key]['count'] = $val->count;
                }
                // $list['Carts'][$key]['count'] = $val->count;
    ...
    public function cartdetial(Request $request)
    {
        ...
        if ($request->has('shippingFee')) {
            $governorate = Governorate::where('shipping_fee', $request->shippingFee)->first();
            if ($governorate) {
                $list['dilivary'] = $governorate->shipping_fee;
                $list['shippingFee'] = $governorate->shipping_fee;
            }
        }
        ...
        if (!is_null($copon)) {
            ...
            $list['order_total'] = (int) ($total + $list['dilivary']) - (int) $capp;
            ...
            $list['order_total'] = ($total + $list['dilivary']);
...
//        // Edited Here
//        // $info->governorate_id = $governorate_id;
//        $info->save();
//
...
////        if($request->has('governorate_id')) {
////            $governorate_id = $request->governorate_id;
////            $gov = Governorate::find(governorate_id);
////            if($gov) {
////                $shipping = $gov->shipping_fee;
////            } else {
...
////            $order->total = Order_Product::where('order_id', $order->id)->sum('price') + $shipping;
////            $order->save();
//
//        } else {
...
//    }
    ...
    public function addorder(Request $request)
    {
        ...
        //$this->homeService->sendPurchaseEvent($totalPayment);
        //} catch (\Exception $e) {}
    ...
    public function dataproductinarray(Request $request)
    {
        ...
        if (!is_null($productId)) {
            // If $productId is an array, find all products
            if (is_array($productId)) {
                $data = Product::find($productId);
            } else {
                // If $productId is a single ID, find that product
                $data = Product::find($productId);
            }
            ...
            if (!is_null($data)) {
                ...
                foreach ($data as $product) {
                    $responseData[] = [
                        ...
                        "card_image" => url($product->image),
                        "rate_num" => $product->rate,
                        "price_new" => $product->price_discount,
                        "stock" => $product->stock,
                    ];
                }
                return response()->json([
            ...
            return response()->json('Please enter a product ID', 400);
        }
    }
