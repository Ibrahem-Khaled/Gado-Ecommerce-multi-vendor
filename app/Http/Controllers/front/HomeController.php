...
//            }
    ...
    public function finnishinfo(Request $request)
    {
        ...
        if (!is_null($order->Order_info)) {
            $order_info = Order_info::find($order->order_info_id);
            $gov = Governorate::find( $order_info->governorate_id );

            // $shippingValue = Governorate::find($order->Order_info()->first()->governorate_id)->shipping_fee;
            $shippingValue = $gov->shipping_fee;
        }
        ...
        // call facebook pixel
        try { $this->homeService->sendPurchaseEvent($total); } catch (\Exception $e) {}
