...
<main>
            ...
            <div class="row">
                <div class="col-md-3">
                    <div class="profile_list">
                        @include('front.profile.customer.links')
                    </div>
                </div>
                <div class="col-md-9">
                    <div class="content_page_shop mb-3 p-0">
                        ...
                        <form class="form-group-custom edit_data row" method="POST" action="{{ route('customer.update_profile') }}">
                            @csrf
                            <div class="col-md-6 mb-3">
                                <label for="firstName" class="form-label"> {{ __('messages.user_name') }} <span>*</span></label>
                                <input type="text" name="name" value="{{ auth()->guard('customer')->user()->name }}" class="form-control" id="firstName"
                                maxlength="50" min="10" required>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for="mobile" class="form-label"> {{ __('messages.Mobile_number') }} <span>*</span></label>
                                <input type="tel" name="phone" class="form-control" id="mobile" value="{{ auth()->guard('customer')->user()->phone }}"
    ...
    @include('front.profile.customer.password_modal')
